...
def compute_weighted_anisotropy(radial, velocity, weights):
    '''This function calculates the anisotropy for the complete set of particles defined
       relative to the halo. Returns both beta defined on magnitude of velocities and 
       dispersion.
    ...
    - NEED TO WEIGHT THE STANDARD DEVIATION AND TEST THIS.
    '''
    ...
    # Calculating the mean the velocities squared. 
    vsm_rad = np.sum(v_rad_abs**2 * weights) / np.sum(weights)
    vsm_tan = np.sum(v_tan_abs**2 * weights) / np.sum(weights)
